...
export default function DashboardAppPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const universityCountResponse = await axios.get('https://localhost:44364/api/AdminDashboard/GetUniversityCount');
        setUniversityCount(universityCountResponse.data);

        const studentCountResponse = await axios.get('https://localhost:44364/api/AdminDashboard/GetStudentCount');
        setStudentCount(studentCountResponse.data);

        const departmentCountResponse = await axios.get('https://localhost:44364/api/AdminDashboard/GetDepartmentCount');
        setDepartmentCount(departmentCountResponse.data);

        const professorCountResponse = await axios.get('https://localhost:44364/api/AdminDashboard/GetProfessorCount');
  ...
  useEffect(() => {
    axios.get('https://localhost:44364/api/News/RecentNewsCount')
      .then(response => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch('https://localhost:44364/api/User');
        ...
        const universityNamesPromises = Object.keys(universitiesCount).map(async universityId => {
          const universityResponse = await fetch(`https://localhost:44364/api/University/GetUniversityById/${universityId}`);
