...
function DetailsProfessor() {
  ...
  const fetchProfessors = async (searchText) => {
    try {
      let url = 'https://localhost:44364/api/Professor';
      if (searchText) {
  ...
  const handleSaveEdit = async () => {
    try {
      const response = await fetch(
        `https://localhost:44364/api/Professor/UpdateProfessor/${editedProfessor.professorId}`,
        {
          method: 'PUT',
          headers: {
  ...
  const handleSaveImageUpload = async () => {
    try {
      ...
      const response = await fetch(
        `https://localhost:44364/api/Professor/UploadProfilePhoto/${editedProfessor.professorId}`,
        {
          method: 'POST',
          body: formData,
        }
  ...
  const handleDeleteProfessor = async () => {
    try {
      const response = await fetch(
        `https://localhost:44364/api/Professor/DeleteProfessor/${editedProfessor.professorId}`,
        {
          method: 'DELETE',
        }
      );
              ...
              <Card sx={{ maxWidth: 345, borderRadius: 0.5 }}>
                <CardActionArea>
                  <CardMedia
                    component="img"
                    height="180"
                    image={`https://localhost:44364/${professor.profilePhotoPath}`}
