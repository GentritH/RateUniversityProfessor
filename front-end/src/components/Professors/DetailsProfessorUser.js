...
function DetailsProfessorUser() {
  ...
  const fetchProfessors = async (searchText) => {
    try {
      ...
      let url = `https://localhost:44364/api/Professor/department/${departmentId}`;
          ...
          professors.map((professor) => (
            <div key={professor.id} className="col-md-4 mb-5">
              <Card sx={{ maxWidth: 345, borderRadius: 0.5  }}>
                <Link  to={`http://localhost:3000/dashboardUser/professorsUser/detProfessorUser/${professor.professorId}`}
                ...
                <CardActions>
                  <Link to={`http://localhost:3000/dashboardUser/professorsUser/listRateProfessorS/${professor.professorId}`}>
