...
import { Container, Typography, Box, Grid, Card, CardContent, CardMedia, Button, Input, List, Link, Breadcrumbs } from '@mui/material';
...
const GetNewsById = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://localhost:44364/api/News/threeLatestCreated')
      .then(response => {
  ...
  return (
    ...
    <Container maxWidth="lg">
    <Box mb={-4} mt={4}>
  <Breadcrumbs separator={<NavigateNextIcon fontSize="small" />} aria-label="breadcrumb" sx={{position: 'relative', padding: 0}}>
    ...
    <Typography color="textPrimary">{news.title}</Typography>
      ...
      <Box mt={5} display="flex" flexDirection="row" justifyContent={'space-between'}>
        <Box sx={{width: '68%'}}>
          <Box mb={0.5}>
            <img
              src={`https://localhost:44364/${news.profilePhotoPath}`}
              alt={news.title}
        ...
        <Box sx={{width: '28%'}}>
          <Grid container spacing={2}>
              <Grid item xs={12} marginBottom={-1.7}>
                <Card sx={{borderRadius: '0%', paddingBottom:1, paddingTop: 1, paddingLeft: 2, fontSize: 30}}>Three latest news</Card>
              </Grid>
            {latestNews.map(item => (
              <Grid item xs={12} sx={{cursor: 'pointer'}} key={item.id} onClick={() => handleCardClick(item.id)}>
