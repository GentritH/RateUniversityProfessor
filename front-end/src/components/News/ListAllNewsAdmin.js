...
import { Container, Typography, Grid, Card, CardContent, CardMedia, CardActions, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, IconButton, Input, InputAdornment, Stack } from '@mui/material';
...
const ListAllNewsAdmin = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://localhost:44364/api/News/GetAllNewsDescByDate')
      .then(response => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`https://localhost:44364/api/News/DeleteNews/${deleteNewsId}`);
  ...
  const handleUpload = async () => {
    try {
      ...
      await axios.post(`https://localhost:44364/api/News/UploadProfilePhoto/${selectedNewsId}`, formData, {
        headers: {
      ...
      const newsToUpdate = updatedNews.find(item => item.id === selectedNewsId);
      if (newsToUpdate) {
    ...
    <div>
      ...
      <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h3">
            ...
            <Link to="http://localhost:3000/dashboard/createNews" style={{ textDecoration: 'none' }}>
        ...
        <Grid container spacing={3}>
          {news.map(item => (
            <Grid item xs={12} sm={6} md={4} key={item.id}>
              <Card>
                  <IconButton className='icbtn-upload' sx={{position: 'absolute'}} onClick={() => handleUploadDialogOpen(item.id)}>
                    <UploadIcon  />
                  </IconButton>
                <CardMedia
                  component="img"
                  height="200"
                  image={`https://localhost:44364/${item.profilePhotoPath}`}
                  alt={item.title}
                />
                ...
                <CardActions>
                  <IconButton onClick={() => handleEditClick(item.id)} color="primary">
                    <EditIcon />
                  </IconButton>
