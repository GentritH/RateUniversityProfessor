...
function DetailsUniversityUser() {
  ...
  const fetchStudentUniversityId = async (email) => {
    try {
      const response = await fetch(`https://localhost:44364/api/User/GetUserByEmail/${email}`);
      ...
      <Grid container spacing={3}>
        {universities.map((university) => (
          <Grid key={university.id} item xs={12} sm={6} md={4} >
            <Card elevation={3}>
            <Link to={`http://localhost:3000/dashboardUser/universitiesUser/detailsUniUser/${university.universityId}`}
                ...
                <strong>Staff Number:</strong> {university.staffNumber}
                      ...
                      <CardActions style={{ borderTop: '1px solid #f0f0f0', background: mixedColor }}>
                        <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                          <Link to={`http://localhost:3000/dashboardUser/universitiesUser/depUniUser/${university.universityId}`}>
