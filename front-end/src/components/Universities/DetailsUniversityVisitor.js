      ...
      <Grid container spacing={3}>
        {universities.map((university) => (
          <Grid key={university.id} item xs={12} sm={6} md={4} >
            <Card elevation={3}>
            <Link to={`http://localhost:3000/detailsUniVisitor/${university.universityId}`}
                ...
                <strong>Staff Number:</strong> {university.staffNumber}
                      ...
                      <CardActions style={{ borderTop: '1px solid #f0f0f0', background: mixedColor }}>
                        <div style={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                          <Link to={`http://localhost:3000/departmentByUniversityVisitor/${university.universityId}`}>
