...
import { ArrowBack as ArrowBackIcon, ExpandMore as ExpandMoreIcon, Edit as EditIcon, Close as CloseIcon ,Visibility as VisibilityIcon } from '@mui/icons-material';
...
function DetailsUniVisitor() {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const response = await fetch(`https://localhost:44364/api/University/UpdateUniversity/${universityId}`, {
        method: 'PUT',
        body: formData
      });
  ...
  const fetchProfessorAndRating = async () => {
    try {
      const professorResponse = await fetch(`https://localhost:44364/api/University/GetUniversityById/${universityId}`);
      const professorData = await professorResponse.json();
      setProfessor(professorData);

      const ratingResponse = await fetch(`https://localhost:44364/api/RateUniversity/GetOverallRatingForUniversities`);
      const ratingData = await ratingResponse.json();
      const professorRating = ratingData.find(rating => rating.universityId === parseInt(universityId));
      if (professorRating) {
    ...
    <Container  style={{ width: '75%', marginTop: '20px', padding: '20px' }}>
      ...
      <Grid container spacing={2} marginTop={2}>
        <Grid item xs={12} md={6}>
          <img src={`https://localhost:44364/${university.profilePhotoPath}`} alt={university.name} style={{ width: '300px', maxHeight: '300px', objectFit: 'cover', borderRadius: '8px' }} />
        </Grid>
        <Grid item xs={12} md={6}>
      ...
      <Box sx={{ alignSelf: 'center'}}> {/* <Typography variant="h6">Ratings</Typography> */} 
      <Box> 
      <Typography> <strong>Overall Rating:</strong> <strong className='strRate'>{overallRating.overallRating}</strong> /5
      </Typography> 
      </Box> 
