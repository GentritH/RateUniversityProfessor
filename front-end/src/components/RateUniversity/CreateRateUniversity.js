...
function CreateRateUniversity() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://localhost:44364/api/RateUniversity/CreateRateUniversity', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        setAlert({ open: true, message: 'Error submitting rating. Please try again.' + responseData.message, severity: 'error' });
      }
    } catch (error) {
