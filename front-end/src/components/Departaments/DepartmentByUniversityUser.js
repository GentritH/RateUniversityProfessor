...
function DepartmentByUniversity() {
  ...
  const fetchDepartments = async () => {
    try {
      const response = await fetch(`https://localhost:44364/api/Department`);
          ...
          <Accordion key={department.departmentId} elevation={3} style={{ margin: '10px 0' }}>
            ...
            <AccordionDetails>
              ...
              <div style={{ marginTop: '0px' }}>
                <Link to={`http://localhost:3000/dashboardUser/departmentsUser/courseDepUser/${department.departmentId}`}>
